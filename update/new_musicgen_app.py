...
def translate_text(text, source_language='vi', target_language='en'):
    ...
    headers = {
        "x-rapidapi-key": "6ea7f5dd10mshff46266473e24fep178607jsn7cadf9b1aa40",  # Replace with your actual RapidAPI key
...
def ui_full(launch_kwargs):
    with gr.Blocks() as demo:
        gr.Markdown(
            ...
            Demo phần mềm tạo sinh cổ nhạc Việt Nam
            """
        )
    with gr.Blocks() as interface:
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    text_temp = gr.Text(label="Input Text", interactive=True)  # Gradio text input
                with gr.Row():
                    submit = gr.Button("Submit")
                    _ = gr.Button("Interrupt").click(fn=interrupt, queue=False)
                with gr.Row():
                    model = gr.Radio(["small"], label="Model", value="facebook/musicgen-stereo-melody", interactive=True)
        ...
        def on_submit(text, melody, model, model_path, decoder, duration, topk, topp, temperature, cfg_coef):
            translated_text = translate_text(text, source_language='vi', target_language='en')
            return predict_full(model, model_path, decoder, translated_text, melody, duration, topk, topp, temperature, cfg_coef)
        
        submit.click(
            on_submit,
            inputs=[text_temp, model, model_path, duration, topk, temperature],
